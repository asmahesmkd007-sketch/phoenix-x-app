...
async function check() {
  const { data: tourneys } = await supabase.from('tournaments').select('id, tr_id, status, phase').eq('status', 'live');
  console.log('Live Tournaments:', JSON.stringify(tourneys, null, 2));
}
check();
